var target = '#target',
  ...
  columns = [
    ...
    {label:'当前状态',col:'width:10%',field:'col2','cssClass':'',style:''},
...
var demoTable = function(target,url,columns){
  ...
  var tableOpts = {
     ...
     */
    queryFields:[
      //{type:types.search,field:'name',placeholder:'请输入商品名称',controlFn:function($node){$node.find('input').val('test');}}//搜索框
...
//{//table 方法
//  onDataLoaded(fun),//fun为重组返回数据方法，可不写，ajax请求直接返回的对象
...
//  onError(fun)//fun为ajax请求出错时调用的方法，默认调alert
//}
